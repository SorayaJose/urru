...
*/
    ...
    {
        $inscripto = Inscripto::find($patinador_id);
    ...
    {
        if (strlen($value) > 1) {
            $this->patinadores = Patinador::where('nombre', 'like', "%{$value}%")->take(5)->get();
    ...
    {
        $patinador = Patinador::find($id);
        if ($patinador) {
            $this->nombre = $patinador->nombre;
            $this->categoria = $patinador->categoria_id; // ajusta el campo según tu modelo
            $this->mostrarSugerencias = false;
        }
    }
