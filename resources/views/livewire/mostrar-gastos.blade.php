<div wire:init="loadRegistros">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <tr>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-800">
                                    <b>{{ $gasto->id }}</b>
                                    - @choice('Cancelado|Activo', $gasto->estado)
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-800">
                                    <b>{{ $gasto->mostrarFecha() }}</b>
                                </div>
                            </td>
                            <td class="px-6 py-4  w-1/6">
                                <div class="text-sm text-gray-800">
                                    @if ($gasto->socio_id)
                                        {{ $gasto->socio->persona->id }} - {{ $gasto->socio->persona->nombre }}
                                        <br>{{ $gasto->socio->persona->apartamento->nombre }}
                                    @else
                                        -
                                    @endif
                                </div>
                            ...
                            <td class="py-4 text-right w-1/6 flex justify-between">
                                <button onclick="window.location.href='{{ route('gastos.edit', $gasto->id) }}'" 
                                ...
                                <button
                                    wire:click="$emit('prueba', {{ $gasto->id }})" 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay gastos a mostrar</div>
            @else
    ...
    <script>
        Livewire.on('prueba', gastoId => {
            ...
            if (result.isConfirmed) {
                ...
                Livewire.emit('eliminarGasto', gastoId)
                Swal.fire({
                    title: "Se elimino el gasto",
