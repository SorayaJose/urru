<div wire:init="loadRegistros">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        ...
        @if (session()->has('mensaje'))
            <div class='text-normal text-green-800 dark:text-green-800 space-y-2 px-6 pt-6'>
                <p class="bg-green-100 border-l-4 border-green-800 text-green-800 font-bold p-4">
                    {{ session('mensaje') }}
                </p>
            </div>
        @endif
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-800">
                                    <b>{{ $socio->persona->apartamento->nombre }}</b>
                            ...
                            <td class="py-4 text-right w-2/6 flex justify-between">
                                <button onclick="window.location.href='{{ route('socios.show', $socio->id) }}'"
                                    class="py-1.5 pl-3 pr-1.5 mr-2  text-sm bg-sky-800 text-white rounded-lg cursor-pointer font-semibold text-center shadow-xs transition-all duration-500 flex items-center hover:bg-sky-900"> 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay socios a mostrar</div>
            @else
    ...
    <script>
        Livewire.on('prueba', socioId => {
            Swal.fire({
                ...
                text: "Un socio eliminado no se puede recuperar",
                icon: "warning",
