<div wire:init="loadRegistros">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <table class='mx-auto max-w-6xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-gray-150">
                    <tr class="text-gray-600 text-left bg-gray-150">
                        <th class="w-2/8 text-left cursor-pointer text-red-800 font-semibold text-sm uppercase px-6 py-4"
                        ...
                        <th class="w-2/8 cursor-pointer text-left text-red-800 font-semibold text-sm uppercase px-6 py-4"
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($escuelas as $escuela)
                        <tr>
                            <td class="w-2/8 px-6 py-4 text-left">
                            ...
                            <td class="w-1/8 py-4 text-right">
                                <button onclick="window.location.href='{{ route('escuelas.edit', $escuela->id) }}'" 
                                ...
                                <button
                                    wire:click="$emit('prueba', {{ $escuela->id }})" 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay escuelas a mostrar</div>
            @else
...
@push('scripts')
    ...
    <script>
        Livewire.on('prueba', escuelaId => {
            Swal.fire({
                title: "¿Eliminar la escuela?",
                text: "Una escuela eliminada no se puede recuperar",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#1F2937",
                cancelButtonColor: "#d33",
                confirmButtonText: "Si, eliminar la escuela!",
                cancelButtonText: 'Cancelar'
                }).then((result) => {
            if (result.isConfirmed) {
                // eliminarl del  servidor
                Livewire.emit('eliminarEscuela', escuelaId)
                Swal.fire({
                    title: "Se eliminó la escuela",
