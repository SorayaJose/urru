<div wire:init="loadRegistros">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <table class='mx-auto max-w-6xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-gray-150">
                    <tr class="text-gray-600 text-left bg-gray-150">
                        <th class="w-1/6 cursor-pointer text-red-800 font-semibold text-sm uppercase 
                            px-6 py-4"
                        ...
                        <th class="w-1/6 cursor-pointer text-left  text-red-800 font-semibold text-sm 
                        uppercase px-6 py-4"
                        ...
                        <th class="w-3/6 cursor-pointer text-left text-red-800 font-semibold text-sm 
                        uppercase px-6 py-4">
                            Descripción
                        </th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($pistas as $pista)
                        <tr>
                            ...
                            <td class="w-1/6 px-6 py-4 text-left">
                                <div class="text-sm text-gray-800">
                                    {{ $pista->direccion }}
                                </div>
                            </td>
                            <td class="w-3/6 px-6 py-4 text-left align-top">
                                <div class="text-sm text-gray-800 whitespace-normal break-words">
                                    {{ $pista->descripcion }}
                                </div>
                            </td>
                            <td class="w-1/6 py-4 text-right">
                                <button onclick="window.location.href='{{ route('pistas.edit', $pista->id) }}'" 
                                ...
                                <button
                                    wire:click="$emit('prueba', {{ $pista->id }})" 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay pistas a mostrar</div>
            @else
...
@push('scripts')
    ...
    <script>
        Livewire.on('prueba', pistaId => {
            Swal.fire({
                title: "¿Eliminar la pista?",
                text: "Una pista eliminada no se puede recuperar",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#1F2937",
                cancelButtonColor: "#d33",
                confirmButtonText: "Si, eliminar pista!",
                cancelButtonText: 'Cancelar'
                }).then((result) => {
            if (result.isConfirmed) {
                // eliminarl del  servidor
                Livewire.emit('eliminarPista', pistaId)
                Swal.fire({
                    title: "Se eliminó la pista",
