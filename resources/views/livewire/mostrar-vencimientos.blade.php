<div wire:init="loadRegistros">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        ...
        @if (count($vencimientos))
            ...
            <table class='mx-auto max-w-6xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-gray-150">
                    <tr class="text-gray-600 bg-gray-150">
                        <th class="cursor-pointer text-left text-gray-900 font-semibold text-sm uppercase px-6 py-4"
                        ...
                        <th class="cursor-pointer text-gray-900 text-left font-semibold text-sm uppercase px-6 py-4"
                        ...
                        <th class="text-gray-900  text-right w-1/6 font-semibold text-sm uppercase px-6 py-4">
                            Importe $
                        </th>
                        <th class=" text-gray-900  text-right w-1/6  font-semibold text-sm uppercase px-6 py-4">
                            Importe U$S
                        </th>
                        <th class="font-semibold text-sm uppercase  text-right w-1/6  px-6 py-4">
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($vencimientos as $vencimiento)
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-800 float-right text-right w-1/6 ">
                            ...
                            <td class="py-4 text-right w-1/6 ">
                                <button onclick="window.location.href='{{ route('vencimientos.edit', $vencimiento->id) }}'" 
                                ...
                                <button
                                    wire:click="$emit('prueba', {{ $vencimiento->id }})" 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay vencimientos a mostrar</div>
            @else
...
@push('scripts')
    ...
    <script>
        Livewire.on('prueba', vencimientoId => {
            Swal.fire({
                title: "¿Eliminar el vencimiento?",
                text: "Un vencimiento eliminado no se puede recuperar",
                icon: "warning",
            ...
            if (result.isConfirmed) {
                // eliminarla  el vencimientos del  servidor
                Livewire.emit('eliminarVencimiento', vencimientoId)
                Swal.fire({
                    title: "Se elimino el vencimiento",
