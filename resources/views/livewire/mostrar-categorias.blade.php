<div wire:init="loadRegistros">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <table class='mx-auto max-w-6xl w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-gray-150">
                    <tr class="text-gray-600 text-left bg-gray-150">
                        <th class="cursor-pointer text-red-900 font-semibold text-sm uppercase px-6 py-4"
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($categorias as $categoria)
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-800">
                                    {{ $categoria->mostrarTipo() }}
                                </div>
                            </td>
                            <td class="py-4 text-right">
                                <button onclick="window.location.href='{{ route('categorias.edit', $categoria->id) }}'" 
                                ...
                                <button
                                    wire:click="$emit('prueba', {{ $categoria->id }})" 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay categorías a mostrar</div>
            @else
...
@push('scripts')
    ...
    <script>
        Livewire.on('prueba', categoriaId => {
            Swal.fire({
