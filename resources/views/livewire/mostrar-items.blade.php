    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        ...
        <div class="p-4 w-full  text-gray-900 border-b border-gray-200 dark:text-gray-100
            ...
            <div class="px-2 py-2 w-full flex items-center">
                ...
                <select wire:model="todos" :value="old('todos')" id="todos"
                    class="font-medium text-sm text-gray-700 dark:text-gray-300 
                    border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    <option value="">Todos</option>
                    <option value=1 selected>Pago entre todos</option>
                    <option value=0>Solo los seleccionados</option>
                        ...
                        <tr>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-800">
                                    <h2 class="text-xl">{{ $item->nombre }}</h2>
                                    <p>Mostrar en recibos: <b>@choice('Si|No', $item->mostrar)</b></p>
                                    <p>Forma de pago: {{ $item->mostrarItem() }} </b></p>
                                    <p>Moneda: <b>{{ $item->moneda }}</b></p>
                                </div>
                            </td>
                            <td class="py-4 text-right">
                                <button onclick="window.location.href='{{ route('items.edit', $item->id) }}'" 
                                ...
                                <button
                                    wire:click="$emit('prueba', {{ $item->id }})" 
        ...
        @else
            @if ($readyToLoad)
                <div class="px-8 py-4">No hay items a mostrar</div>
            @else
...
@push('scripts')
    ...
    <script>
        Livewire.on('prueba', itemId => {
            Swal.fire({
                title: "¿Eliminar el item del recibo?",
                text: "Un item  eliminado no se puede recuperar y puede alterar los recibos ya emitidos",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#1F2937",
                cancelButtonColor: "#d33",
                confirmButtonText: "Si, eliminar item del recibo!",
                cancelButtonText: 'Cancelar'
                }).then((result) => {
            if (result.isConfirmed) {
                // eliminarl del  servidor
                Livewire.emit('eliminarItem', itemId)
                Swal.fire({
                    title: "Se elimino el item del recibo",
